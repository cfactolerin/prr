...
Each tool is a standalone script in `bin/`. No wrapper dispatcher — just run the tool directly.

### Setup: `bin/prr setup`

Interactive configuration wizard. Generates `config/prr.yml`.
...
Base repo path [~/fuga/git]: <enter to keep>
GitHub username []: crisfactolerin
Claude timeout in seconds [600]: <enter to keep>
Codex timeout in seconds [900]: <enter to keep>
Arbiter rounds [3]: <enter to keep>
Min disk space in GB [10]: <enter to keep>
Jira base URL []: https://fuga.atlassian.net
Jira API token []: ****

Config written to config/prr.yml
```

Behavior:
- If config already exists, shows current value in brackets. Press enter to keep it.
- Sensitive values (API tokens) are masked on display.
- After setup, validates that required tools are available (`claude`, `codex`, `gh`).
...
Config is resolved with this precedence (highest wins):

1. **CLI flags** (`--claude-timeout 300`)
2. **Environment variables** (`PRR_CLAUDE_TIMEOUT=300`)
3. **Config file** (`config/prr.yml`)
4. **Defaults** (hardcoded in the tool)

### Config file: `config/prr.yml`
...
jira_base_url: https://fuga.atlassian.net
...
Every config key maps to a `PRR_` prefixed env var:

| Config key        | Env var                | Example                          |
|-------------------|------------------------|----------------------------------|
| base_repo_path    | PRR_BASE_REPO_PATH     | ~/fuga/git                       |
| tmp_path          | PRR_TMP_PATH           | tmp/reviews                      |
| claude_timeout    | PRR_CLAUDE_TIMEOUT     | 600                              |
| codex_timeout     | PRR_CODEX_TIMEOUT      | 900                              |
| arbiter_rounds    | PRR_ARBITER_ROUNDS     | 3                                |
| min_disk_space_gb | PRR_MIN_DISK_SPACE_GB  | 10                               |
| github_user       | PRR_GITHUB_USER        | crisfactolerin                   |
| jira_base_url     | PRR_JIRA_BASE_URL      | https://fuga.atlassian.net       |
| jira_api_token    | PRR_JIRA_API_TOKEN     | token_here                       |
| jira_email        | PRR_JIRA_EMAIL         | cris@fuga.com                    |

This lets engineers who prefer env vars (e.g., via `.envrc` or shell profile) skip the config file entirely.
...
tools/
  bin/
    prr                       # PR review tool (standalone executable)
  lib/
    prr/
      cli.rb                  # argument parsing, --help, subcommand routing
      config.rb               # config file + env var + CLI flag resolution
      setup.rb                # interactive config wizard
  ...
  config/
    prr.yml                   # generated by `prr setup`, gitignored
    prompts/
